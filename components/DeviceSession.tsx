...
const DeviceSession: React.FC<DeviceSessionProps> = ({ device, onToggleTheme, isDarkMode }) => {
    const [logs, setLogs] = useState<LogEntry[]>([{
        id: 'CRASH-2023-10-05-001',
        timestamp: '2023-10-05 14:30:45',
        level: 'info',
        message: 'NullPointerException in onCreate method',
        tag: null
    }, {
        id: 'Cr13',
        timestamp: '2023-10-05 14:30:45',
    ...
    useEffect(() => {
        // 1. Join Session
        apiClient.joinDeviceSession(device.deviceId);

        // 2. Fetch Historical Logs
        setIsLoadingLogs(true);
        apiClient.getLogs(device, device.deviceId).then((fetchedLogs) => {
            setLogs(fetchedLogs);
                    ...
                    <div className="flex bg-slate-100 dark:bg-slate-800/50 p-1 rounded-xl overflow-x-auto no-scrollbar max-w-[200px] sm:max-w-md lg:max-w-2xl">
                        {[
                            ...
                            <button
                                ...
                                className={`px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === tab.id
